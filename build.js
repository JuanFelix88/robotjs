...
function build(runtime, version, abi) {
  return new Promise(function (resolve, reject) {
    ...
    console.log('Building robotjs for ' + runtime + ' v' + version + '>>>>');
    if (process.platform === 'win32') {
      ...
      process.env.gyp_robotjs_runtime = runtime;
      process.env.gyp_robotjs_abi = abi;
      process.env.gyp_robotjs_platform = process.platform;
      process.env.gyp_robotjs_arch = arch;
    }
...
function tarGz(runtime, abi) {
  const FILES_TO_ARCHIVE = {
    linux: ['build/Release/robotjs.node'],
    win32: ['build/Release/robotjs.node'],
    darwin: ['build/Release/robotjs.node'],
  };
  const tarPath =
    'prebuilds/robotjs-v' +
    pkg.version +
